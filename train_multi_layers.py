...
def main(config):
    ...
    replaced_layers = [elem['name'] for elem in config['pruning']['pruning_plan']]
    hint_layers = config['pruning']['hint']
    unfreeze_layers = config['pruning']['unfreeze']
    student = WrappedStudent(teacher, config)
    student.replace(replaced_layers)
    student.register_hint_layers(hint_layers)
    student.unfreeze(unfreeze_layers)
    logger.info(student)
    ...
    optimizer = config.init_obj('optimizer',
                                module_optim,
                                filter(lambda x: x.requires_grad, student.student.parameters())
                                )
