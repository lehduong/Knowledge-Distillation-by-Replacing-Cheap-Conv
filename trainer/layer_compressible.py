    ...
    def train_epoch(self, epoch, layer_name, compress_rate, lr):
        ...
        for batch_idx, (data, target) in enumerate(self.train_data_loader):
            ...
            if batch_idx % self.log_step == 0:
                self.writer.add_scalars("mIoU/" + layer_name.replace('.', '_'), {str(lr): self.train_iou_metrics.get_iou()}, batch_idx)
                self.writer.add_scalars("loss/" + layer_name.replace('.', '_'), {str(lr): loss.item()}, batch_idx)
                iou_gap = self.train_teacher_iou_metrics.get_iou() - self.train_iou_metrics.get_iou()
                self.writer.add_scalars("student_teacher_iou_gap/" + layer_name.replace('.', '_'), { str(lr): iou_gap}, batch_idx)
                self.logger.info(
